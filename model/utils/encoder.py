...
from .video_encoder import (VideoEncoderConfig,
    VideoEncoderModel, VideoImageProcessor)
...
class Videollama3VisionEncoder(nn.Module):
    ...
    def __init__(self, vision_encoder, args, delay_load=False):
        ...
        if not delay_load:
            ...
            # uncertain whether flash-attention-2 is supported during inference phase.
            self.attn_implementation = 'sdpa' # 'eager'
            self.cfg_only = VideoEncoderConfig.from_pretrained(self.vision_encoder_name)
    ...
    def load_model(self, args):
        ...
        self.image_processor = VideoImageProcessor.from_pretrained(self.vision_encoder_name)

        # merge_size is fixed to 1 for STAGE1, STAGE1.5, STAGE2, STAGE3 in encoder and can be modified in connector.
        self.cfg_only = VideoEncoderConfig.from_pretrained(self.vision_encoder_name)

        self.vision_encoder = VideoEncoderModel.from_pretrained(
